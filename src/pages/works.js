...
const WorksPage = ({ data }) => {
  const { works, institutions, page } = data
    ...
    <>
      <Layout>
        <Wrapper className={styles.portfolio} id="main">
          <ContentHeader
            title={page?.seo?.seo_title || `All`}
            display={false}
          />
          {noresults && <NoResults tagName={page?.seo?.seo_title || `All`} />}
          <Loading
            elemId="spinner"
...
export const query = graphql`
  query ListView {
    works: allStrapiWork(sort: { productionDate: DESC }) {
      nodes {
        id
        title
        slug
...
export const Head = ({ location, data, pageContext }) => {
  ...
  seoTitle = page?.seo?.seo_title || `All Works`
  seoDesc = page?.seo?.seo_description || null
